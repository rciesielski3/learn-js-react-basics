...
import { controlledExamples } from '@/data/react/controlled';
...
export default function ControlledVsUncontrolled() {
  ...
  React.useEffect(() => {
    ...
    for (const ex of controlledExamples) {
      try {
        const result = ex.run();
    ...
    <div className="flex items-center justify-center min-h-screen p-6 bg-gradient-to-b from-gray-800 to-gray-500">
      <div className="mt-6 p-6 max-w-3xl mx-auto border border-gray-400 bg-gray-700 rounded">
        <SectionHeader
          ...
          description="Understand the difference between controlled and uncontrolled React form components with visual examples and code."
        />
        {controlledExamples.map((example) => (
          <ExampleBlock
            key={example.id}
            id={example.id}
            name={example.name}
